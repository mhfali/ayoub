...
class Canvas:
    ...
    def run(self, **kwargs):
        ...
        for k in kwargs.keys():
            if k in ["query", "user_id", "files", "original_query", "original_language", "translated_query_for_retrieval"] and kwargs[k]:
                if k == "files":
                    self.globals[f"sys.{k}"] = self.get_files(kwargs[k])
                else:
                    self.globals[f"sys.{k}"] = kwargs[k]
        
        # Special handling for language translation context
        if kwargs.get("original_query") and kwargs.get("original_language"):
            # Store the original query for LLM response generation
            self.globals["sys.original_user_query"] = kwargs.get("original_query")
            self.globals["sys.user_language"] = kwargs.get("original_language")
            # Use the original query for the user input display
            self.add_user_input(kwargs.get("original_query"))
        else:
