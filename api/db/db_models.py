...
class ChatLog(DataBaseModel):
    id = CharField(max_length=32, primary_key=True)
    tenant_id = CharField(max_length=32, null=False, index=True)
    user_id = CharField(max_length=255, null=False, help_text="user_id", index=True)
    dialog_id = CharField(max_length=32, null=True, index=True)
    conversation_id = CharField(max_length=32, null=True, index=True)
    question = TextField(null=False, help_text="User question")
    response = TextField(null=True, help_text="System response")
    is_flagged = BooleanField(default=False, help_text="Is question flagged as unrelated", index=True)
    log_type = CharField(max_length=32, null=False, default="normal", help_text="normal|flagged", index=True)
    flag_reason = TextField(null=True, help_text="Reason for flagging")
    kb_ids = JSONField(null=True, default=[], help_text="Knowledge base IDs used")
    tokens_used = IntegerField(default=0, index=True)
    response_time = FloatField(default=0, help_text="Response time in seconds")
    source = CharField(max_length=16, null=True, help_text="completion|ask|agent", index=True)
    metadata = JSONField(null=True, default={}, help_text="Additional metadata")
